...
export class UserRepository extends BaseRepository{
    ...
    async create(model: UserReadModel, options?: IQueryOptions): Promise<UserReadModel>{
    ...
    async findByEmail(email: string, options?: IQueryOptions): Promise<UserReadModel | null> {
        const {entityManager} = this.parseOptions(options);
        const repo = entityManager.getRepository(UserEntity);
